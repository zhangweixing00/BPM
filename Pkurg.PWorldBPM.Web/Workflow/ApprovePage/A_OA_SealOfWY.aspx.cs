...
[BPM(AppId = "3025")]
public partial class Workflow_ApprovePage_A_OA_SealOfWY : A_WorkflowFormBase
{
    ...
    private void UpdateWFParams()
    {
        ...
        if (K2_TaskItem.ActivityInstanceDestination.Name == "部门负责人意见")
        {
            if (cbAP.Checked)
            {
                dataFields.Add("AssiPresident", Workflow_Common.GetRoleUsers(StartDeptId + "," + Countersign1.Result, "主管助理总裁"));
            }
            else
            {
                dataFields.Add("AssiPresident", "noapprovers");
            }
            if (cbVP.Checked)
            {
                dataFields.Add("VicePresident", Workflow_Common.GetRoleUsers(StartDeptId + "," + Countersign1.Result, "主管副总裁"));
            }
            else
            {
            ...
            {
                dataFields.Add("VicePresident", Workflow_Common.GetRoleUsers(WYCode, "总裁"));
            }
            else
            {

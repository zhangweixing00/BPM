...
public partial class Workflow_ApprovePage_A_JC_ProjectTenderCityCompany : UPageBase
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            if (!string.IsNullOrEmpty(Request.QueryString["sn"]))
            {
                ...
                if (nodeName.Value.Contains("集团") && nodeName.Value!="集团招标委员会意见")
                {
                ...
                else if (nodeName.Value.Contains("集团招标委员会意见"))
                {
                    tbCompanyCommittee.Visible = false;
                    tbPresident.Visible = true;
                }
                else
                {
                    ...
                    tbPresident.Visible = cblIsImpowerProject.SelectedItem.Value == "0" ? true : false;
                }
            }
            else
            {
    ...
    public string GetViewUrl()
    {
        ...
        ViewUrl = StrTemp + "ViewPage/V_JC_ProjectTenderCityCompany.aspx?ID=" + Request.QueryString["id"];
        return ViewUrl;
    }
}

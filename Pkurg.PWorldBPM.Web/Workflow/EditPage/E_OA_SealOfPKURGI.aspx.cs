...
public partial class Workflow_EditPage_E_OA_SealOfPKURGI : UPageBase
{
    public string className = "Workflow_EditPage_E_OA_SealOfPKURGI";

    SealOfPKURGI Eitems = new SealOfPKURGI();
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            {
                FormId = BPMHelp.GetSerialNumber("OA_IYZ_");
    ...
    private void InitCheckBoxList()
    {
        if (!string.IsNullOrEmpty(Request.QueryString["id"]))
        {
            ...
            SealOfPKURGIInfo formDataInfo = Eitems.Get(FormId);
    ...
    private void BindFormData()
    {
        try
        {
            SealOfPKURGIInfo obj = Eitems.Get(ViewState["FormID"].ToString());
    ...
    private SealOfPKURGIInfo SaveData(string ID, string wfStatus)
    {
        SaveWFParams();
    ...
    private bool SaveWorkFlowInstance(SealOfPKURGIInfo obj, string WfStatus, DateTime? SumitTime, string WfInstanceId)
    {
        bool result = false;
    ...
    private string SaveWFParams()
    {
        ...
        if (string.IsNullOrEmpty(GetRoleUsers(GroupCode, "公章管理员")))
        {
            flag = false;
        ...
        if (this.cbPresident.Checked)//暂不需要总裁审批
        {
        ...
        if (1 == 1)
        {
            //LeaderTemp = GetRoleUsers(GroupOfficeCode, "部门负责人");//改为主管领导复核
            LeaderTemp = GetRoleUsers(GroupOfficeCode, "主管副总裁");
    ...
    protected void Save_Click(object sender, EventArgs e)
    {
        ...
        SealOfPKURGIInfo obj = SaveData(id, "00");
        if (obj != null)
        {
    ...
    protected void Submit_Click(object sender, EventArgs e)
    {
        ...
        SealOfPKURGIInfo obj = SaveData(id, "02");
        if (obj != null)
        {
            //开启流程
            WorkflowHelper.CurrentUser = "founder\\" + _BPMContext.CurrentUser.LoginId;
            AppDict appInfo = new Pkurg.BPM.Services.AppDictService().GetByAppId("3009");
            if (appInfo == null)
            {
                Alert("提交失败");
                return;
            ...
            WorkflowHelper.StartProcess(@"K2Workflow\OA_SealOfPKURGI", id, dataFields, ref wfInstanceId);
            if (wfInstanceId > 0)
            {

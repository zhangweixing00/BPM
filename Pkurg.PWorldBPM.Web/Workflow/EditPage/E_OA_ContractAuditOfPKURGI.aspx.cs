...
public partial class Workflow_EditPage_E_OA_ContractAuditOfPKURGI : UPageBase
{
    ContractAuditOfPKURGI Eitems = new ContractAuditOfPKURGI();
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            {
                //投资公司的编码前缀为I
                FormId = BPMHelp.GetSerialNumber("OA_IHT_");
    ...
    private void InitCheckBoxList()
    {
        ...
        if (!string.IsNullOrEmpty(Request.QueryString["id"]))
        {
            ...
            ContractAuditOfPKURGIInfo formDataInfo = Eitems.Get(FormId);
    ...
    private void BindFormDate()
    {
        ...
        {
            //根据formid得到合同表单数据
            ContractAuditOfPKURGIInfo obj = Eitems.Get(ViewState["FormID"].ToString());
    ...
    /// <returns></returns>
    private ContractAuditOfPKURGIInfo SaveData(string ID, string wfStatus)
    {
        SaveWFParams();
        ContractAuditOfPKURGIInfo obj = null;
        try
        {
            ...
            if (obj == null)
            {
                obj = new ContractAuditOfPKURGIInfo();
            ...
            //存储数据
            //LINQ
            //var info = BizContext.OA_ContractAuditOfPKURGI.FirstOrDefault(x => x.FormID == FormId);
            //if (info == null)
            //{
            //    BizContext.OA_ContractAuditOfPKURGI.InsertOnSubmit(new Pkurg.PWorldBPM.Business.BIZ.OA_ContractAuditOfPKURGI()
            //    {
            //        FormID = FormId
            //    });
            //}
    ...
    /// <returns></returns>
    private bool SaveWorkFlowInstance(ContractAuditOfPKURGIInfo obj, string WfStatus, DateTime? SumitTime, string WfInstanceId)
    {
        bool result = false;
    ...
    protected void Save_Click(object sender, EventArgs e)
    {
        ...
        ContractAuditOfPKURGIInfo obj = SaveData(id, "00");
        if (obj != null)
        {
    ...
    protected void Submit_Click(object sender, EventArgs e)
    {
        ...
        ContractAuditOfPKURGIInfo obj = SaveData(id, "02");
        if (obj != null)
        {
            //开启流程
            WorkflowHelper.CurrentUser = "founder\\" + _BPMContext.CurrentUser.LoginId;
            AppDict appInfo = new Pkurg.BPM.Services.AppDictService().GetByAppId("3005");
            if (appInfo == null)
            {
                Alert("提交失败");
                return;
            ...
            WorkflowHelper.StartProcess(@"K2Workflow\OA_ContractAuditOfPKURGI", id, dataFields, ref wfInstanceId);
            if (wfInstanceId > 0)
            {

...
public partial class Workflow_EditPage_E_OA_ContractAuditOfEToI : UPageBase
{
    ContractAuditOfEToI Eitems = new ContractAuditOfEToI();
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            {
                //投资公司的编码前缀为I
                FormId = BPMHelp.GetSerialNumber("OA_EIHT_");
    ...
    private void InitCheckBoxList()
    {
        ...
        if (!string.IsNullOrEmpty(Request.QueryString["id"]))
        {
            ...
            ContractAuditOfEToIInfo formDataInfo = Eitems.Get(FormId);
    ...
    private void BindFormDate()
    {
        ...
        {
            //根据formid得到合同表单数据
            ContractAuditOfEToIInfo obj = Eitems.Get(ViewState["FormID"].ToString());
    ...
    /// <returns></returns>
    private ContractAuditOfEToIInfo SaveData(string ID, string wfStatus)
    {
        SaveWFParams();
        ...
        try
        {
            ...
            if (obj == null)
            {
                obj = new ContractAuditOfEToIInfo();
            ...
            //存储数据
            //LINQ
            //var info = BizContext.OA_ContractAuditOfEToI.FirstOrDefault(x => x.FormID == FormId);
            //if (info == null)
            //{
            //    BizContext.OA_ContractAuditOfEToI.InsertOnSubmit(new Pkurg.PWorldBPM.Business.BIZ.OA_ContractAuditOfEToI()
            //    {
            //        FormID = FormId
            //    });
            //}
    ...
    /// <returns></returns>
    private bool SaveWorkFlowInstance(ContractAuditOfEToIInfo obj, string WfStatus, DateTime? SumitTime, string WfInstanceId)
    {
        bool result = false;
    ...
    protected void Save_Click(object sender, EventArgs e)
    {
        ...
        ContractAuditOfEToIInfo obj = SaveData(id, "00");
        if (obj != null)
        {
    ...
    protected void Submit_Click(object sender, EventArgs e)
    {
        ...
        ContractAuditOfEToIInfo obj = SaveData(id, "02");
        if (obj != null)
        {
            //开启流程
            WorkflowHelper.CurrentUser = "founder\\" + _BPMContext.CurrentUser.LoginId;
            AppDict appInfo = new Pkurg.BPM.Services.AppDictService().GetByAppId("3006");
            if (appInfo == null)
            {
                Alert("提交失败");
                return;
            ...
            WorkflowHelper.StartProcess(@"K2Workflow\OA_ContractAuditOfEToI", id, dataFields, ref wfInstanceId);
            if (wfInstanceId > 0)
            {

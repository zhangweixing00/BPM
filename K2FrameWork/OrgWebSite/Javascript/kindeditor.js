...
KE.lang = {
	...
	pager : '插入分页符' , 
	// add by dengCong begin 添加锚点功能
	anchor : '锚点',
	invalidAnchorName : ' 请输入锚点名称'
	// add by dengCong end
};
...
KE.setting = {
	...
	items : [
		...
		'superscript'
		, '-', 'fullscreen', ,'image', 'media',  'link', 'unlink','anchor','table', 'hr','pager', 'emoticons',  'selectall','source'
	...
	htmlTags : {
		...
		a : ['class', 'href', 'target', 'name', 'style' , 'title'],
		embed : ['src', 'width', 'height', 'type', 'loop', 'autostart', 'quality', 'style','flashvars', 'align', 'allowscriptaccess', '/'],
	...
	pagerCode : '<div class=\"ke-pager\"></div>'
};
...
KE.format = {
	getUrl : function(url, mode, host, pathname) {
		...
		// delete by dengCong begin  修正url为空时不自动添加/portalm
		//if (url.match(/^\//)) {
			//url = host + getRealPath(url.substr(1));
		//} else if (!url.match(/^\w+:\/\//)) {
			//url = host + getRealPath(pathname + '/' + url);
		//}
		// delete by dengCong end

		// add by dengCong begin  url为空时为避免跳转错误url改为#,不做跳转
		...
		if (mode == 'relative') {
			...
			//alert("absolute : "+url+"   substr:"+url.substr(host.length));
...
KE.util = {
	...
	getFullHtml : function(id) {
		...
		/*
		if (KE.g[id].globalPath) {
	      html += '<link href="' + KE.g[id].globalPath + 'styles/style_page.css" rel="stylesheet" type="text/css" />';
	    }
	    */
		...
		if (KE.g[id].cssPath) {
		 ...
		 html += '<link href="' + KE.g[id].skinsPath + 'pager.css" rel="stylesheet" type="text/css" />';
		html += '</head>';
	...
	getMediaImage : function(id, type, url, width, height, autostart,flashvars) {
		...
		if (typeof flashvars != 'undefined') hash.flashvars = flashvars;
	...
	getMediaEmbed : function(id, type, url, width, height, autostart,flashvars) {
		
		if(type !=null )
		{
		   if(type =="flv" || type =="flash")
		   {
			   var temp_flvplayer='<embed type="application/x-shockwave-flash" src="/portalm/public/player/flvplayer.swf"'; 
		        temp_flvplayer+='id="flashPlayer" name="flashPlayer" bgcolor="#000000" quality="high"  allowfullscreen="true"';
		        temp_flvplayer+='flashvars="'+url+'"    width="'+width+'" height="'+height+'" />';
		        return temp_flvplayer;
		   }
		}
		else{
			if (flashvars.match(/\.(flv)(\?|$)/i))
			{
			  
				temp_flvplayer='<embed type="application/x-shockwave-flash" src="/portalm/public/player/flvplayer.swf"'; 
		        temp_flvplayer+='id="flashPlayer" name="flashPlayer" bgcolor="#000000" quality="high"  allowfullscreen="true"';
		        temp_flvplayer+='flashvars="'+flashvars+'"    width="'+width+'" height="'+height+'" />';
	...
	selfclick : function(id, cmd, cb) {
        //CR add
    	KE.plugin[cmd].setcallback(cb);
        KE.util.click(id, cmd);
    },
    formatWord : function(id)
    {  //CR add
   		var str= KE.util.getData(id);
    	str = str.replace(/<meta(\n|.)*?>/ig, "");
		str = str.replace(/<!(\n|.)*?>/ig, "");
		str = str.replace(/<style[^>]*>(\n|.)*?<\/style>/ig, "");
		if(typeof(KE.g[id].noFilter)!="string" || KE.g[id].noFilter != "script"){
...
KE.layout = {
	hide : function(id) {
	try {
			KE.util.getIframeDoc(KE.g[id].dialog).body.innerHTML = ""; //jim
	    } catch (e) {}
		var g = KE.g[id];
		g.dialogStack = [];
		g.hideDiv.innerHTML = '';
...
KE.dialog = function(arg){
	...
	this.hide = function() {
		try {
			KE.util.getIframeDoc(KE.g[arg.id].dialog).body.innerHTML = ""; //jim
	    } catch (e) {}
	...
	this.show = function() {
		...
		if (typeof arg.html != "undefined") {
			...
			if (typeof arg.file == "undefined") {
				 //CR add
				 var temp_cmd= arg.cmd;
		        	if(temp_cmd=="imageupload")
		        	{
		        		temp_cmd="image";
		        	}
		        	var dialogUrl = KE.g[id].pluginsPath + temp_cmd + '.jsp?editid=' + KE.g[arg.id].id;
		        	if (KE.g[arg.id].openid) {
		        		dialogUrl += "&id=" + KE.g[id].openid;
		        		
		        		KE.g[arg.id].openid = null;
		    		}
		        	iframe.src = dialogUrl+"&dialogtype="+arg.cmd;
...
KE.create = function(id, mode) {
	...
	var selectImageForWebkit = function(e) {
	 ...
	 /*-- add listener,Jim,2009/11/25 --*/
    KE.event.add(iframe, 'blur', new Function('KE.syncData("' + id + '")'));
    KE.event.add(iframeDoc, 'blur', new Function('KE.syncData("' + id + '")'));
...
//jim, synchronize data 20091125
KE.syncData = function(id) {
		var obj = KE.g[id];
		var html = KE.util.execGetHtmlHooks(id, obj.iframeDoc.body.innerHTML);
			html = html.replace(/^\s*<br[^>]*>\s*$/ig, '');
		
		if (obj.filterMode) {
	        obj.srcTextarea.value = KE.format.getHtml(obj.iframeDoc.body.innerHTML, obj.htmlTags);
	    } else {
	        obj.srcTextarea.value = KE.format.getHtml(html, obj.htmlTags, obj.urlType);
	        //obj.srcTextarea.value = KE.format.getHtml(obj.iframeDoc.body.innerHTML,obj.htmlTags, obj.urlType);
	    }
}
// jim, insert html 20091125
KE.insertHtml = function(id, html) {
	  KE.util.focus(id);
	  KE.util.selection(id);
...
KE.plugin['plainpaste'] = {
	click : function(id) {
		...
		this.dialog = new KE.dialog({
			id : id,
			cmd : 'plainpaste',
			file : 'plainpaste.jsp?id=' + id + '&ver=' + KE.version,
			width : 400,
			height : 300,
			loadingMode : true,
...
KE.plugin['wordpaste'] = {
	click : function(id) {
		...
		this.dialog = new KE.dialog({
			id : id,
			cmd : 'wordpaste',
			file : 'wordpaste.jsp?id=' + id + '&ver=' + KE.version,
			width : 400,
			height : 300,
			loadingMode : true,
...
KE.plugin['fontname'] = {
	click : function(id) {
		var fontName = {
			'SimSun' : '宋体',
			'SimHei' : '黑体',
			'FangSong_GB2312,FangSong' : '仿宋体',
			'KaiTi_GB2312,KaiTi' : '楷体',
			'NSimSun' : '新宋体',
			'Arial' : 'Arial',
...
KE.plugin['image'] = {
	...
	click : function(id) {
		...
		//CR add
		if(typeof KE.g[id].imageColumn == "undefined")
		{
		    alert("栏目ID不存在");
		    return ;
		}
		else { 
			KE.g[id].openid = KE.g[id].imageColumn;
		}
		KE.util.selection(id);
		this.dialog = new KE.dialog({
			id : id,
			cmd : 'image',
			file : 'image/image.jsp?id=' +id+'&cid=' + KE.g[id].openid+ '&dialogtype=image&ver=' + KE.version,
			width : 400,
			height : 230,
			loadingMode : true,
			title : KE.lang['image'],
	...
	check : function(id) {
		...
		if(type ==2)
		{
			...
			if (!url.match(/\.(jpg|jpeg|gif|bmp|png|JPG|JPEG|GIF|BMP|PNG)(\s|$)/i)) {
	...
	exec : function(id) {
		...
		if (type == 1) {
		 ...
		 this.insert({id:id, url:url, title:title, width:width, height:height, border:0,align:align});
		}
	},
	insert : function(filedata) {
		var id=filedata.id;
		var url=filedata.url;
		var width=filedata.width;
		var height=filedata.height;
		var title=filedata.title;
		var align=filedata.align;
		var border=filedata.border;
...
KE.plugin['link'] = {
	...
	click : function(id) {
		if(typeof KE.g[id].otherColumn == "undefined")
		{
		    alert("栏目ID不存在");
		    return ;
		}
		else {
			KE.g[id].openid = KE.g[id].otherColumn;
		}
		KE.util.selection(id);
		this.dialog = new KE.dialog({
			id : id,
			cmd : 'link',
			file : 'link/link.jsp?id=' +id+'&cid=' + KE.g[id].openid+ '&dialogtype=link&ver=' + KE.version,
			width : 400,
			height : 175,
			loadingMode : true,
			title : KE.lang['link'],
	...
	exec : function(id) {
		...
		var dialogDoc = KE.util.getIframeDoc(this.dialog.iframe);
		var type = KE.$('type', dialogDoc).value;
		var title=KE.$('otherTitle', dialogDoc).value;
		var url = KE.$('url', dialogDoc).value;
	    var name=KE.$('filename', dialogDoc).value;
	    ...
	    this.insert({id:id,name:name,url:url,target:target,title:title});
		 
	},
    insert : function(filedata) {
    	var id=filedata.id;
    	var url=filedata.url;
     	var target=filedata.target;
   	    var title=filedata.title;
    	...
    	{
    		var html='<a   title="'+title+'" href="'+url+'" target="'+target+'" >'
    	    if(name.length >0)
    	    {
    	        html+=name+"</a>";
    	    }else{	
...
KE.plugin['media'] = {
	init : function(id) {
		...
		KE.g[id].getHtmlHooks.push(function(html) {
			return html.replace(/<img[^>]*class="?ke-\w+"?[^>]*>/ig, function($0) {
				...
				var flashvars = $0.match(/style="[^"]*;?\s*flashvars:\s*(\d+)/i) ? RegExp.$1 : 0;
				width = width || ($0.match(/width="([^"]+)"/i) ? RegExp.$1 : 0);
				height = height || ($0.match(/height="([^"]+)"/i) ? RegExp.$1 : 0);
				flashvars = flashvars || ($0.match(/flashvars="([^"]+)"/i) ? RegExp.$1 : 0);
				if ($0.match(/\s+title="([^"]+)"/i)) {
					...
					flashvars = hash.flashvars || '';
					
					// html+= PortalUI.mediaShow(null,filedata.url,filedata.fId,filedata.ftype,
				    //    		filedata.fGroup,filedata.auto,filedata.width,filedata.height);
					return KE.util.getMediaEmbed(id, null, src, width, height, autostart,flashvars);
				}
			});
		});
		KE.g[id].setHtmlHooks.push(function(html) {
			return html.replace(/<embed[^>]*type="([^"]+)"[^>]*>/ig, function($0, $1) {
				...
				var src = $0.match(/\s+src="([^"]+)"/i) ? RegExp.$1 : '';
				//FILE
				var flashvars = $0.match(/\s+flashvars="([^"]+)"/i) ? RegExp.$1 : 0;
				var width = $0.match(/\s+width="([^"]+)"/i) ? RegExp.$1 : 0;
				var height = $0.match(/\s+height="([^"]+)"/i) ? RegExp.$1 : 0;
				var autostart = $0.match(/\s+autostart="([^"]+)"/i) ? RegExp.$1 : 'false';
				return KE.util.getMediaImage(id, null, src, width, height, autostart,flashvars);
			});
		});
	},
	click : function(id) {
		//CR add
		if(typeof KE.g[id].mediaColumn == "undefined")
		{
		    alert("栏目ID不存在");
		    return ;
		}
		else {
			KE.g[id].openid = KE.g[id].mediaColumn;
		}
		KE.util.selection(id);
		this.dialog = new KE.dialog({
			id : id,
			cmd : 'media',
			file : 'media.jsp?id=' +id+'&cid=' + KE.g[id].openid+ '&dialogtype=media&ver=' + KE.version,
			width : 400,
			height : 200,
			loadingMode : true,
			title : KE.lang['media'],
	...
	check : function(id, url, width, height) {
        ...
        if(type == 2)
        {
        	if (!url.match(/^\w+:\/\/.{3,}\.(swf|flv|wmv|avi|asf|rm|rmvb)(\?|$)/i)) {
	...
	exec : function(id) {
		...
		//CR add
		var fId= KE.$('fid', dialogDoc).value;
        var ftype= KE.$('ftype', dialogDoc).value;
        var fGroup= KE.$('fGroup', dialogDoc).value; 
        if(ftype !=null && ftype.length >0)
        {
        	if(ftype =='flv')
        	{
        	   var pathname = (location.pathname.match(/^(\/.*)\//) ? RegExp.$1 : '');
        		url= pathname+"/"+fId+"."+ftype;
        	}else
        	{
        		url=url+"."+ftype;
        	}
		...
		this.insert({id:id, url:url,fId:fId,ftype:ftype,fGroup:fGroup,width:width, height:height, auto:autostart});
	},
	insert : function(filedata) {
		var html = KE.util.getMediaImage(filedata.id, null, filedata.url, filedata.width, filedata.height, filedata.autostart,"file="+filedata.url);
		KE.util.insertHtml(filedata.id, html);
		KE.layout.hide(filedata.id);
		KE.util.focus(filedata.id);
    }
};
...
KE.plugin['imageupload'] = {
		...
		click : function(id) {
			...
			if(typeof KE.g[id].showTab != "undefined")
			{ 
				showTabs = "&showTab="+KE.g[id].showTab;
				tempHeight=150;
				KE.lang['image']="附件上传";
			}
			...
			this.dialog = new KE.dialog({
				id : id,
				cmd : 'imageupload',
				file : 'image/image.jsp?id=' +id+showTabs+'&cid=' + KE.g[id].openid+ '&dialogtype=imageupload&ver=' + KE.version,
				width : tempWidth,
				height : tempHeight,
				loadingMode : true,
				title : KE.lang['image'],
		...
		exec : function(id) {
			...
			if (type == 1) {
				...
				var title = KE.$('imgTitle', dialogDoc).value;
				var alignElements = dialogDoc.getElementsByName('align');
				var fid = KE.$('callbackfid', dialogDoc).value;
				var align = '';
				for (var i = 0, len = alignElements.length; i < len; i++) {
					if (alignElements[i].checked) {
						align = alignElements[i].value;
			 ...
			 this.insert({id:id,fid:fid, url:url, title:title, width:width, height:height, border:0,align:align});
			}
		},
		insert : function(filedata) {
...
KE.plugin['anchor'] = {
	getSelectedNode : function(id) {
		...
		var startNode = g.keRange.startNode;
		if (startNode.nodeType != 1) return;
		if (startNode.tagName.toLowerCase() != 'a') return;
		//alert(startNode.className);
		if (!startNode.className.match(/^ke-\w+/i)) return startNode;
	},
	init : function(id) {
		var self = this;
	...
	click : function(id) {
		...
		this.dialog = new KE.dialog({
			id : id,
			cmd : 'anchor',
			file : 'anchor/anchor.jsp?id=' +id+ '&dialogtype=anchor&ver=' + KE.version,
			width : 400,
			height : 90,
			loadingMode : true,
			title : KE.lang['anchor'],
			yesButton : KE.lang['yes'],
			noButton : KE.lang['no']
		});
		this.dialog.show();
	...
	exec : function(id) {
		...
		var dialogDoc = KE.util.getIframeDoc(this.dialog.iframe);
		var anchorName=KE.$('anchorName', dialogDoc).value;
                var anchorTitle=KE.$('anchorTitle', dialogDoc).value;

		if( anchorName.length == 0 ) {
			alert(KE.lang['invalidAnchorName']);
			KE.$('anchorName', dialogDoc).focus();
			return false;
		}
		
		var html = '<a name="'+anchorName+'" title="'+anchorTitle+'" class="ke-anchor">&nbsp;&nbsp;</a>';
		KE.util.insertHtml(id, html);
